...
if __name__ == '__main__':
    ...
    parser.add_argument("--colors", "-c", nargs=3, default=['blue', 'red', "yellow"], required=False, help="Colors for train and eval plots respectively.")
    # parser.add_argument("--colors", "-c", nargs=6, default=['red']*3 + ['blue']*3, required=False, help="Colors for the 6 log plots. Default is first 3 blue and last 3 red.")
    parser.add_argument("--data_key", "-d", type=str, required=True)
    # parser.add_argument("--data_keys", "-d", nargs='+', type=str, required=True, help="Data keys to extract from tensorboard. E.g., train_return and eval_return.")
    ...
    if args.plot_mean_std:
        ...
        # ax = plt.gca()
        # for log, name in zip(args.input_log_files, args.human_readable_names):
        #     for idx, data_key in enumerate(args.data_keys):
        #         color = args.colors[idx]  # assign color based on order of data_key
        #         scalars = extract_tensorboard_scalars(log, data_key)
        #         plot_scalars(ax, scalars, data_key, f"{name} ({data_key})", color)
        # ax.legend()
        ...
        for log, name, color in zip(args.input_log_files, args.human_readable_names, args.colors):
            try:
                ...
                args.data_key = "Eval_AverageReturn"
                scalars = extract_tensorboard_scalars(log, args.data_key)
    ...
    if has_names:
        plt.legend()
    plt.savefig(f'{"-".join(args.human_readable_names)}.png')
    # plt.show()